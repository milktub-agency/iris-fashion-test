((window) => {
  ...
  const position = (el) => {
    ...
    return {
      ...
      left:
        elemOffset.left -
        parentOffset.left -
        (parseInt(elemStyle.marginLeft) || 0),
    };
  };
  ...
  const getCascadedStyle = (el) => {
    ...
    Array.prototype.slice
      .call(clone.querySelectorAll('input[type="radio"]'))
      .forEach((el) => {
    ...
    for (const prop in currentStyle) {
      if (
        isNaN(prop) &&
        (typeof currentStyle[prop] === "string" ||
    ...
    if (Object.keys(style).length < 3) {
      ...
      for (const prop in currentStyle) {
        if (!isNaN(prop)) {
          style[
            currentStyle[prop].replace(/-\w/g, (s) => {
              return s.toUpperCase().replace("-", "");
            })
    ...
    if (!style.margin && style.marginLeft === "auto") {
      ...
      style.marginLeft === style.marginBottom
    ) {
    ...
    if (
      ...
      const marginLeft =
        posLeft - (parseInt(style.left) || 0) - (parseInt(style.right) || 0);
      const marginRight =
