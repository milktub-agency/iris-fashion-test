...
(function (global, factory) {
  ...
  const hcSticky = function (elem, userSettings) {
    ...
    const setOptions = (options) => {
      ...
      if (
        Helpers.isEmptyObject(options) &&
        !Helpers.isEmptyObject(STICKY_OPTIONS)
      ) {
        // nothing to set
        return;
      }
      ...
      if (mediaQueries) {
        ...
        if (STICKY_OPTIONS.mobileFirst) {
          for (const width in mediaQueries) {
            if (
              window_width >= width &&
              !Helpers.isEmptyObject(mediaQueries[width])
            ) {
          ...
          for (let i = queriesArr.length - 1; i >= 0; i--) {
            ...
            if (
              ...
              !Helpers.isEmptyObject(query[breakpoint])
            ) {
    ...
    const getStickyCss = (el) => {
      ...
      css["width"] =
        cascadedStyle.width !== "auto"
          ? cascadedStyle.width
          : css.boxSizing === "border-box" || css.MozBoxSizing === "border-box"
          ? el.offsetWidth + "px"
          : computedStyle.width;
    ...
    const Sticky = {
      ...
      release: (args) => {
        ...
        if (
          ...
          ((typeof args.top === "undefined" &&
            typeof args.bottom === "undefined") ||
            (typeof args.top !== "undefined" &&
              (parseInt(Helpers.getStyle(elem, "top")) || 0) === args.top) ||
            (typeof args.bottom !== "undefined" &&
              (parseInt(Helpers.getStyle(elem, "bottom")) || 0) ===
                args.bottom))
        ) {
          return;
        }
        ...
        elem.style.left =
          args.stop === true ? Sticky.css.left : Spacer.positionLeft + "px";
        elem.style.width =
          position !== "absolute" ? Sticky.css.width : Spacer.width;
        ...
        if (elem.classList) {
          ...
          elem.className = elem.className.replace(
            new RegExp(
              "(^|\\b)" +
                STICKY_OPTIONS.stickyClass.split(" ").join("|") +
                "(\\b|$)",
              "gi"
            ),
            " "
    ...
    const Spacer = {
      ...
      init: () => {
        ...
        Spacer.offsetLeft =
          Helpers.offset(elem).left - (parseInt(elemStyle.marginLeft) || 0);
    ...
    const calcSticky = () => {
      ...
      stickTo_document =
        ...
        (STICKY_OPTIONS.stickTo === "document" ||
          (STICKY_OPTIONS.stickTo.nodeType &&
            STICKY_OPTIONS.stickTo.nodeType === 9) ||
          (typeof STICKY_OPTIONS.stickTo === "object" &&
            STICKY_OPTIONS.stickTo instanceof
      ...
      calcStickyHeight = () => {
        const height =
          elem.offsetHeight +
          (parseInt(Sticky.css.marginTop) || 0) +
          (parseInt(Sticky.css.marginBottom) || 0);
      ...
      container_offsetTop = !stickTo_document
        ? Helpers.offset(container).top
        : 0;
      ...
      // top
      options_top =
        isNaN(STICKY_OPTIONS.top) && STICKY_OPTIONS.top.indexOf("%") > -1
          ? (parseFloat(STICKY_OPTIONS.top) / 100) * window_height
          : STICKY_OPTIONS.top;

      // bottom
      options_bottom =
        isNaN(STICKY_OPTIONS.bottom) && STICKY_OPTIONS.bottom.indexOf("%") > -1
      ...
      stick_bottom =
        isNaN(STICKY_OPTIONS.bottomEnd) &&
        STICKY_OPTIONS.bottomEnd.indexOf("%") > -1
          ? (parseFloat(STICKY_OPTIONS.bottomEnd) / 100) * window_height
          : STICKY_OPTIONS.bottomEnd;

      top_limit =
        container_offsetTop - options_top + stick_top + sticky_offsetTop;
    };
    ...
    const runSticky = () => {
      ...
      bottom_limit =
        container_offsetTop + container_height - options_top - stick_bottom;
      ...
      if (offset_top > top_limit) {
        ...
        if (
          bottom_limit +
            options_top +
            (largerSticky ? options_bottom : 0) -
            (STICKY_OPTIONS.followScroll && largerSticky ? 0 : options_top) <=
          offset_top +
            sticky_height -
            stick_top -
            (sticky_height - stick_top >
              window_height - (top_limit - stick_top) &&
            STICKY_OPTIONS.followScroll
              ? (bottom_distance = sticky_height - window_height - stick_top) >
                0
                ? bottom_distance
                : 0
              : 0)
          ...
          Sticky.release({
            ...
            bottom:
              elemParent_offsetTop +
              elemParent.offsetHeight -
              bottom_limit -
              options_top,
          });
          ...
          if (scroll_dir === "down") {
            // scroll down
            if (
              sticky_window_top + sticky_height + options_bottom <=
              window_height + 0.9
            ) {
              // stick on bottom
            ...
            // scroll up
            if (
              Math.ceil(sticky_window_top + stick_top) < 0 &&
              Sticky.position === "fixed"
            ) {
    ...
    const initSticky = () => {
      ...
      if (
        ...
        Helpers.getStyle(elem, "display") === "none"
      ) {
        disableSticky();
        return;
      }
    ...
    const resizeSticky = () => {
      ...
      if (STICKY_OPTIONS.onBeforeResize) {
        STICKY_OPTIONS.onBeforeResize.call(
          elem,
          Object.assign({}, STICKY_OPTIONS)
        );
      }
    ...
    const resize_cb = !STICKY_OPTIONS.resizeDebounce
      ? resizeSticky
      : Helpers.debounce(resizeSticky, STICKY_OPTIONS.resizeDebounce);
